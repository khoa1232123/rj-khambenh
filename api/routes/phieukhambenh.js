...
router.get('/', async (req, res) => {
  ...
  try {
    const records = await Phieukhambenh.find().populate({
      path: 'hosobenhnhan',
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    console.log(record);
    res.status(200).send('Bạn đã tạo phieu kham benh mới thành công!!!');
  } catch (err) {
    res.status(500).json(err);
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    const updateRecord = await Phieukhambenh.findByIdAndUpdate(
      req.params.id,
      {
        $set: req.body,
      },
